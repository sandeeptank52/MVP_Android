...
class ForgotPassFragment : BaseFragment(R.layout.login_forgotpass_fragment) {
        ...
        val mailDisposable = forgotPassBinding?.forgotPassTextInputEditTextMail?.textChanges()
            ...
            ?.map {
                ...
                if (mailCheck) {
                    ...
                    lifecycleScope.launch {
                        ...
                        when (val existMail = loginModel.remoteRepo.isMailExist(SendEmail(it.toString()))) {
                            is RetrofitResult.Success -> {
                                ...
                                if (stateMailField) setErrorMailField(null)
                                else setErrorMailField(resources.getString(R.string.error_form_field_email_not_registered))
                            }
                    ...
                    setErrorMailField(resources.getString(R.string.error_form_fields_incorrect_email))
                    stateMailField = false
                    stateMailFieldSubject.onNext(stateMailField)
                }
            }?.subscribe()

        val mailCheckDisposable = stateMailFieldSubject.subscribe {
            forgotPassBinding?.forgotPassButtonSend?.isEnabled = it
        }
    ...
    private fun setErrorMailField(error: String?) {
        ...
        if (error == null) {
            forgotPassBinding?.forgotPassTextInputLayoutMail?.hint =
                resources.getString(R.string.login_hint_email)
        } else {
            forgotPassBinding?.forgotPassTextInputLayoutMail?.hint = error
        }
    }
    private fun setVisibleProgressBar(isVisible: Boolean) {
        if (isVisible)
            forgotPassBinding?.forgotPassProgressBar?.visibility = View.VISIBLE
        else forgotPassBinding?.forgotPassProgressBar?.visibility = View.GONE
    }
    private fun setEnableInterface(enable: Boolean) {
        ...
        if (enable)
            forgotPassBinding?.forgotPassProgressBar?.visibility = View.GONE
        else forgotPassBinding?.forgotPassProgressBar?.visibility = View.VISIBLE
    }
    ...
    private fun showErrorDialog(errorType: RetrofitError) {
        ...
        forgotPassBinding?.root?.let { Snackbar.make(it, text, Snackbar.LENGTH_SHORT).show() }
    }
